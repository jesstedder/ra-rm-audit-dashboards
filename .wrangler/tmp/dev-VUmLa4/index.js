...
// .wrangler/tmp/bundle-0iWCOG/strip-cf-connecting-ip-header.js
...
// src/lib/rm/client.ts
var RMApiError = class extends Error {
...
function createClient(credentials) {
  ...
  async function getAll(path, queryString, isRetry = false) {
    ...
    do {
      ...
      if (response.status === 401 && !isRetry) {
        cachedToken = null;
        return getAll(path, queryString, true);
      }
...
// src/lib/rm/charges.ts
async function getChargeTypes(client) {
  return client.getAll("/ChargeTypes");
}
__name(getChargeTypes, "getChargeTypes");
var PET_FEE_NAMES = /* @__PURE__ */ new Set(["pet fee", "pf"]);
function findPetFeeChargeTypes(chargeTypes) {
  return chargeTypes.filter((ct) => PET_FEE_NAMES.has(ct.Name.toLowerCase()));
}
__name(findPetFeeChargeTypes, "findPetFeeChargeTypes");
async function getRecurringChargesByTypeIds(client, chargeTypeIds) {
  if (chargeTypeIds.length === 0)
    return [];
  const pages = await Promise.all(
    chargeTypeIds.map(
      (id) => client.getAll("/RecurringCharges", `filters=ChargeTypeID,eq,${id}`)
    )
  );
  return pages.flat();
}
__name(getRecurringChargesByTypeIds, "getRecurringChargesByTypeIds");

// src/lib/rm/pets.ts
async function getAllPets(client) {
  return client.getAll("/Pets");
}
__name(getAllPets, "getAllPets");
function getTenantIdsWithPets(pets) {
  return new Set(pets.map((p) => p.TenantID));
}
__name(getTenantIdsWithPets, "getTenantIdsWithPets");

// src/lib/rm/tenants.ts
async function getAllTenants(client) {
  return client.getAll("/Tenants");
}
__name(getAllTenants, "getAllTenants");
async function getTenantsByIds(client, ids) {
  if (ids.size === 0)
    return [];
  const all = await getAllTenants(client);
  return all.filter((t) => ids.has(t.TenantID));
}
__name(getTenantsByIds, "getTenantsByIds");
function tenantDisplayName(tenant) {
  return `${tenant.FirstName} ${tenant.LastName}`;
}
__name(tenantDisplayName, "tenantDisplayName");

// src/lib/rm/audits/pet-fee.ts
async function findTenantsWithPetFeeButNoPet(client) {
  ...
  const [recurringCharges, pets] = await Promise.all([
...
__name(findTenantsWithPetFeeButNoPet, "findTenantsWithPetFeeButNoPet");
...
// .wrangler/tmp/bundle-0iWCOG/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0iWCOG/middleware-loader.entry.ts
