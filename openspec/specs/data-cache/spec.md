...
Requirements for the Cloudflare Cache API wrapper and cache refresh endpoint.
...
The worker SHALL expose `POST /api/cache/refresh` (behind the existing auth middleware) that busts the `rm-data`, `ps-profiles`, and `pet-fee-audit` cache entries and returns `{ ok: true }`.

#### Scenario: Refresh triggered
- **WHEN** authenticated user POSTs to `/api/cache/refresh`
- **THEN** all cache entries are deleted and `{ ok: true }` is returned with HTTP 200
