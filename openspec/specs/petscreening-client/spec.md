# Capability: PetScreening Client

Requirements for the PetScreening API client library.
...
`getAllPsProfiles(env)` SHALL fetch all pages of `/web_api/v1/property_managers/{PMID}/pet_profiles` with query params `q[with_status_for_manager]=all&q[sorted_by]=shared_at`, accumulating results until a page returns fewer items than the page size (indicating the last page). The function SHALL return an array of `PsProfile` objects including all fields required by both the real-time endpoints and the analytics ETL.

The `PsProfile` type SHALL include:
- Existing fields: `id`, `name`, `species`, `kind`, `status`, `pawScore`, `publicToken`, `ownerName`, `unitAddress`, `unitNumber`
- New fields: `score` (number — raw fractional score from `screening_info.score`), `sharedAt` (string | null — ISO timestamp from `property_manager.shared_at`), `createdAt` (string — date string from top-level `created_at`), `assistanceType` (string | null), `archived` (boolean), `archiveReason` (string | null), `highlights` (string[] — array of flag label strings from `screening_info.highlights`), `missingRequiredFields` (string[] — from `screening_info.missing_required_fields`)
...
#### Scenario: All new fields populated
- **WHEN** a profile has `score: 0.5`, `shared_at: "2024-06-19T14:27:02.311Z"`, `highlights: ["Breed", "Weight"]`, and `archived: false`
- **THEN** the returned `PsProfile` has `score: 0.5`, `sharedAt: "2024-06-19T14:27:02.311Z"`, `highlights: ["Breed", "Weight"]`, `archived: false`

#### Scenario: Missing optional fields
- **WHEN** a profile has no `property_manager.unit` and no `screening_info`
- **THEN** `unitNumber: null`, `sharedAt: null`, `highlights: []`, `missingRequiredFields: []`, `score: 0`
