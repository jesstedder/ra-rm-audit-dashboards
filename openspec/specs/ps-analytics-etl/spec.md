# Capability: PS Analytics ETL

Requirements for the daily Cloudflare Cron ETL that snapshots PetScreening and RentManager data into D1.

---

## Requirement: D1 analytics schema

The system SHALL create a Cloudflare D1 database named `ps-analytics` with binding `ps_analytics` and the following tables, applied via a migration file at `migrations/0001_analytics_schema.sql`:
